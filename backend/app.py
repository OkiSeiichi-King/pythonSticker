...
def stickers_layout():
    ...
    cash_pos_y = 0
    angle = 0
    temp_images = [Polygon([(0,0),(0,pear_logo_dims[1]),(pear_logo_dims[0],pear_logo_dims[1]),(pear_logo_dims[0], 0)])]
    for sticker in sticker_data:
        index +=1
        for cash_pos_x in range(0, int(pear_book_dims[0])):
            for angle in range(0, 360):
                affinity.rotate(sticker['polygon'], angle, origin='center', use_radians=False)
                affinity.translate(sticker['polygon'], xoff=cash_pos_x, yoff=0)
                flag = False
                for image in temp_images:
                    if sticker['polygon'].contains(image):
                        flag = True

                if flag == False:
                    sticker_response.append({'size': [int(sticker['width']) , int(sticker['height'])], 'position': [cash_pos_x, cash_pos_y], 'rotation': 0, 'i': index })
                    temp_images.append(sticker['polygon'])
                    break
        if cash_pos_x > pear_book_dims[0]:
            cash_pos_x = 0
            cash_pos_y += 1
